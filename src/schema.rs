...
///
/// Its value is a ruleset element - basically identical to output elements, apart from certain properties which may be interpreted differently or are required when output generally omits them.
pub enum NfCmd {
    /// Add a new ruleset element to the kernel.
    Add(NfListObject),
    /// Replace a rule.
    ///
    /// In [RULE][Rule], the **handle** property is mandatory and identifies the rule to be replaced.
    Replace(Rule),
    ...
    /// Insert an object.
    ///
    /// This command is identical to [add][NfCmd::Add] for rules, but instead of appending the rule to the chain by default, it inserts at first position.
    /// If a handle or index property is given, the rule is inserted before the rule identified by those properties.
    Insert(NfListObject),
    /// Delete an object from the ruleset.
    ///
    /// Only the minimal number of properties required to uniquely identify an object is generally needed in the enclosed object.
    /// For most ruleset elements, this is **family** and **table** plus either **handle** or **name** (except rules since they don’t have a name).
    Delete(NfListObject), // TODO: ADD_OBJECT is subset of NfListObject
    /// List ruleset elements.
    ///
    /// The plural forms are used to list all objects of that kind, optionally filtered by family and for some, also table.
    List(NfListObject),
...
pub struct Chain {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    /// The chain’s handle. In input, it is used only in [delete command][NfCmd::Delete] as alternative to **name**.
    pub handle: Option<u32>,
...
pub struct Map {
    ...
    // map/set comment attribute requires at least nftables 0.9.7 and kernel 5.10
    pub comment: Option<String>,
}
...
pub struct MetainfoObject {
    ...
    ///
    /// If supplied in (libnftables) library input, the parser will verify the json_schema_version value to not exceed the internally hardcoded one (to make sure the given schema is fully understood).
    /// In future, a lower number than the internal one may activate compatibility mode to parse outdated and incompatible JSON input.
